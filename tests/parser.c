...
void test_crypto_seed_parse() {
    ...
    crypto_seed seed;
    bcr_error err = parse_seed(len, raw, &seed);
...
void test_crypto_psbt_parse() {
    ...
    bcr_error err = parse_psbt(len, raw, &psbt);
    TEST_ASSERT_EQUAL(bcr_error_tag_noerror, err.tag);
...
void test_crypto_eckey_parse() {
    ...
    crypto_eckey eckey;
    bcr_error err = parse_eckey(len, raw, &eckey);
    TEST_ASSERT_EQUAL(bcr_error_tag_noerror, err.tag);
...
void test_crypto_hdkey_parse_1() {
    ...
    crypto_hdkey hdkey;
    bcr_error err = parse_hdkey(len, raw, &hdkey);
    TEST_ASSERT_EQUAL(bcr_error_tag_noerror, err.tag);
...
void test_crypto_hdkey_parse_2() {
    ...
    TEST_ASSERT_EQUAL(0x00, hdkey.key.derived.keydata[0]);
}
